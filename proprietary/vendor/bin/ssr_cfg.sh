...
while [ "${index}" -lt "${MAX_NUM_SUBSYS}" ] ; do
  ...
  if [ -d "${SUBSYS_PATH}${index}" ] ; then
    ...
    LOG "Name : $name"
    if [ "$name" != "" -a "$name" != "${SUBSYS_MODEM}" -a "$name" != "${SUBSYS_ADSP}" ] ; then
...
case $restartlevel in
  ...
  ;;
  "8")
  setprop persist.sys.ssr.restart_level "${SUBSYS_ADSP}"
  ;;
  "16")
  setprop persist.sys.ssr.restart_level "${SUBSYS_OTHERS}"
  ;;
  "12")
  setprop persist.sys.ssr.restart_level "${SUBSYS_MODEM} ${SUBSYS_ADSP}"
  ;;
  "20")
  setprop persist.sys.ssr.restart_level "${SUBSYS_MODEM} ${SUBSYS_OTHERS}"
  ;;
  "24")
  setprop persist.sys.ssr.restart_level "${SUBSYS_ADSP} ${SUBSYS_OTHERS}"
  ;;
  "28")
